  ...
  with st.expander("Custom Date Range"):
    ...
    if custom_dates:
      custom_start = st.date_input(
        "Start Date", value=date(today.year, today.month, 1), key="custom_start"
      )
      custom_end = st.date_input("End Date", value=today, key="custom_end")
...
with col1:
  ...
  if category_totals:
    ...
    wedges, texts, autotexts = ax.pie(
      sizes, labels=labels, autopct="%1.1f%%", colors=sns.color_palette("Set2"), startangle=90
    )
...
if date_totals:
  ...
  points = (
    ...
    .encode(x="Date", y="Total Amount Spent", tooltip=["Date", "Total Amount Spent"])
    .interactive()
  )
  text = points.mark_text(align="center", baseline="bottom", color="#808080", dy=-7).encode(
    ...
    with st.spinner("Analyzing your spending patterns..."):
      try:
        insights = st.session_state.receipt_extraction_agent.get_simple_insights(
          st.session_state.receipt_handler, earliest_date, latest_date
        )
